...
| 脚本 | 功能 |
|------|------|
| [sausg_calc.py](.claude/skills/sausg/scripts/sausg_calc.py) | 自动运行SAUSG进行结构计算 |
| [sausg_result.py](.claude/skills/sausg/scripts/sausg_result.py) | 读取并显示计算结果 |
| [sausg_open.py](.claude/skills/sausg/scripts/sausg_open.py) | 用指定模块打开模型 |
...
```bash
python .claude/skills/sausg/scripts/sausg_open.py <模型路径> [模块名称] [软件目录]
```
...
```bash
# 用默认模块(OpenSAUSG)打开
python .claude/skills/sausg/scripts/sausg_open.py Test/Example.ssg

# 用隔震模块打开
python .claude/skills/sausg/scripts/sausg_open.py Project/Example.ssg 隔震

# 用减震模块打开
python .claude/skills/sausg/scripts/sausg_open.py Project/Example.ssg zeta

# 用指定版本的软件打开
python .claude/skills/sausg/scripts/sausg_open.py Project/Example.ssg 非线性 "D:\SAUSG2027"
```
...
```bash
python .claude/skills/sausg/scripts/sausg_calc.py <模型路径> [软件目录] [--no-cleanup]
```
...
```bash
# 自动搜索软件并计算（默认会清理旧文件）
python .claude/skills/sausg/scripts/sausg_calc.py Test/Example.ssg

# 指定软件目录
python .claude/skills/sausg/scripts/sausg_calc.py Project/Example2.ssg "D:\SAUSG2026"

# 不清理旧文件
python .claude/skills/sausg/scripts/sausg_calc.py Test/Example.ssg --no-cleanup
```
...
```bash
python .claude/skills/sausg/scripts/sausg_result.py <模型目录> [模型名称]
```
...
python .claude/skills/sausg/scripts/sausg_result.py MulProject/P1 P1

# 读取Project目录下的计算结果
python .claude/skills/sausg/scripts/sausg_result.py Project/Example
```
...
```bash
# 第1个模型计算完成后再执行第2个
python .claude/skills/sausg/scripts/sausg_calc.py Project/Model1.ssg
# 等待上述计算完成后，再执行下一个
python .claude/skills/sausg/scripts/sausg_calc.py Project/Model2.ssg
# 等待计算完成后再执行下一个
python .claude/skills/sausg/scripts/sausg_calc.py Project/Model3.ssg
```
## 支持的终端环境
